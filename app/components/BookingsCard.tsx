...
const BookingsCard: React.FC<BookingsCardProps> = ({ booking, signedImages, detailView }) => {
  ...
  React.useEffect(() => {
    ...
    const handleKey = (e: KeyboardEvent) => {
      if (e.key === "ArrowRight") {
        ...
        setZoomIdx((prev) => (prev - 1 + (signedImages?.length || 1)) % (signedImages?.length || 1));
      }
    };
  ...
  const address = [spot.address, spot.city, spot.state].filter(Boolean).join(", ");
    ...
    <>
      <Card className="w-full m-auto md:max-w-7xl mb-6 overflow-hidden border-2 shadow-md">
        ...
        <CardContent className="px-6 py-4">
                  ...
                  img.signedUrl ? (
                    ...
                    >
                      <Image
                        ...
                        className={`h-40 w-56 object-cover rounded-lg shadow-sm ${img.is_primary ? 'border-4 border-blue-500' : ''}`}
                        width={224}
                        height={160}
                      />
                    </button>
          ...
          {spot.description && spot.description.trim() !== '' &&
            <Alert className="mb-4 bg-amber-50 border-amber-200">
              ...
              <AlertDescription className="text-amber-800">
                <span className="font-medium block mb-1">Parking Instructions:</span>
                {spot.description}
              </AlertDescription>
            </Alert>
          }
          ...
          {spot.price_per_day && (
            <div className="mb-4 p-3 bg-slate-50 rounded-md">
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="font-bold text-slate-900">${spot.price_per_day.toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-slate-600">Service fee (3%):</span>
                <span className="text-slate-600">${(spot.price_per_day * 0.03).toFixed(2)}</span>
              </div>
          ...
          {/* Action buttons */}
          <div className={`flex ${detailView ? 'flex-col md:flex-row' : 'flex-col'} gap-3 mt-4`}>
            {detailView ? (
              <Button
                variant="default"
                asChild
              ...
              <Button
                ...
                className="w-full"
              >
                <Link href={`/dashboard/rentedbookings/${booking.id}`}>
                  <Info className="h-4 w-4 mr-2" />
                  See Booking Details
                </Link>
              </Button>
            ...
            <Button
              variant="secondary"
              className={`w-full ${detailView ? 'md:w-3/4' : ''} bg-green-600 hover:bg-green-700 text-white`}
              onClick={() => {
                if (address) {
              ...
              {signedImages.length > 1 && (
                <div className="absolute inset-0 flex items-center justify-between pointer-events-none">
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      setZoomIdx((zoomIdx - 1 + signedImages.length) % signedImages.length);
                    }}
                  >
                    &#8592;
                  </button>
